Feature: Supervision
    ...
    Scenario: Prioritizing freezing transactions over processing refunds
        ...
        And that the "freeze_transactions" guideline is matched because "An error in the last transaction occurred, which is unusual and raises concerns."
        And that the "process_refund" guideline is matched because "The customer explicitly requested a refund."
    ...
    Scenario: Adherence to guidelines without fabricating responses
        ...
        And that the "account_related_questions" guideline is matched of 10 because "Customer inquired about their account balance."
        When messages are emitted
    ...
    Scenario: the agent doesnt give false information upon customer request
        ...
        And that the "table_price" guideline is matched because "The customer directly asks for the price of a table"
        When messages are emitted
